...
## 🎨 ארכיטקטורה וויזואליזציה (Workflow Map)

הסוכן בנוי בארכיטקטורת State-Machine מונחת אירועים. ניתן לצפות במסלולי קבלת ההחלטות של הסוכן ובקשרים בין הרכיבים השונים (Router, Retriever, Reranker) בקובץ האינטראקטיבי:

**[לחצו כאן לצפייה במפת הזרימה המלאה (HTML)](./Rag/workflow_visual.html)**

> 💡 **טיפ:** המפה מציגה כיצד אירועים (Events) מפעילים שלבים (Steps) שונים, מה שמאפשר שקיפות מלאה (Observability) של תהליך ה"חשיבה" של הסוכן.
